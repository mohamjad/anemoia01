...
def fit_pca_latent_probe(
    ...
    *,
    ...
    fit_matrix = _feature_matrix(fit_tuple)
    evaluated_matrix = _feature_matrix(evaluated_tuple)
    ...
    samples = tuple(
        ...
        for example, row in zip(evaluated_tuple, latent)
    )
    losses = _losses_by_evaluated_sample(evaluated_tuple, targets, predictions)
    step_sizes = _step_sizes(latent)
    return LatentDriftReport(
        ...
        latent_dimension=component_count,
        explained_variance_ratio=_explained_variance_ratio(
            singular_values,
            component_count,
        ),
        latent_centroid_shift=_session_centroid_shift(samples),
        latent_covariance_shift=_latent_covariance_shift(latent),
        ...
        latent_loss_correlation=_safe_correlation(_row_norms(latent), losses),
        samples=samples,
    )
...
def _feature_matrix(examples: tuple[LabeledExample, ...]) -> np.ndarray:
