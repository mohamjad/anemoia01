...
def test_load_falcon_h2_trials_reads_trial_table(tmp_path) -> None:
    ...
    assert len(loaded) == 2
    assert loaded[0].sample_id == "falcon_h2:held_out_calib:2023-04-17:trial-11"
    assert loaded[1].cue == "d>e"
    assert loaded[1].block_num == 6


def test_load_falcon_h2_trials_sample_id_includes_split(tmp_path) -> None:
    path = tmp_path / "sub-T5-held-in-calib_ses-20230417.nwb"
    with h5py.File(path, "w") as handle:
        trials = handle.create_group("intervals/trials")
        trials.create_dataset("cue", data=[b"abc"])
        trials.create_dataset("start_time", data=[0.0])
        trials.create_dataset("stop_time", data=[1.0])
        trials.create_dataset("id", data=[11])

    held_in = load_falcon_h2_trials(path, IngestSplit.HELD_IN_CALIB)[0]
    minival = load_falcon_h2_trials(path, IngestSplit.MINIVAL)[0]

    assert held_in.sample_id != minival.sample_id
    assert held_in.sample_id.startswith("falcon_h2:held_in_calib:")
    assert minival.sample_id.startswith("falcon_h2:minival:")
